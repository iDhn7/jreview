...
플레이 라이트 페이지 이동시 다시 시작하는 로직으로 메모리 초기화
5페이지 까지 진행
헤드 내용은 실질 컴퓨터 헤드로 설정
코드 작성 정리
...
페이지 선택
스크롤 내림으로 데이터 로딩
스크롤 내림으로 로딩 안되는 데이터 대응 리스트 선택 40개
로딩 후 전체 데이터 개수 확인
리스트 업체 클릭 후 크롤링 데이터 수집
페이지 변경시 브라우저 종료후  홈페이지 접속후 페이지 선택부터 반복 5페이지 까지(브라우저 종료는 메모리 누적으로 종료하는 로직 사용,페이지 5페이지는 네이버가 키워드당 최대 검색 횟수로 설정)
...
def GetItemList(keyword,pageNo,csv_file_path) :
    # 1. 플레이라이트를 시작합니다.
    with sync_playwright() as p:
        ...
        context               = browser.new_context(
            user_agent        = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/149.0.0.10 Safari/537.36",
        ...
        url                   = f"https://map.naver.com/p/search/전주%20{keyword}%20맛집"
        # https://map.naver.com/p/search/전주%20전북대%20맛집
        print(url)
        print(f"{keyword} 지역의 맛집 정보를 크롤링합니다")
        page.goto(url)
       ...
       # iframe 전환
        ...
        try :
            print(f"{pageNo} 페이지 이동중...")
            search_frame.locator("a.mBN2s").get_by_text(re.compile(f"^{pageNo}$")).click()
            page.wait_for_timeout(5000)
            print(f"{pageNo} 페이지 이동 완료...")
            ...
            # 로딩을 위해 앞에 선택 하는 로직 사용 (스크롤사용 로직으로 데이터 로딩 안됨) 최소 개수는 확인 X 40개 정도하니 로딩 완료
            ...
            print(f"없는 페이지 { pageNo }입니다.")
            return
        # 브라우저 종료
        browser.close()        
    
region_list             = ["전북대","한옥마을","객사","신시가지"]
...
for region in region_list :
    for pageNo in range(1,6) :
        GetItemList(region,pageNo,csv_file_path)
        time.sleep(90)
